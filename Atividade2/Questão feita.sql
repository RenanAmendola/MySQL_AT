...
insert into tb_classes (classe,desricao) values 
...
insert into tb_personagens (nome,cidade,ataque,defesa,id_classe) values ('thor','argasd',2000,500,1);
...
select * from tb_personagens
right join tb_classes on tb_classes.id = tb_personagens.id_classe;

#busca detalhada
select tb_personagens.nome, tb_classes.classe, tb_classes.descricao from tb_personagens
inner join tb_classes on tb_classes.id = tb_personagens.id_classe;

select tb_personagens.nome, tb_classes.classe, tb_classes.descricao from tb_personagens
inner join tb_classes on tb_classes.id = tb_personagens.id_classe where classe = 'elfo(a/e)';
